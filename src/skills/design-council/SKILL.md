...
- **Machine gates run FIRST and are not relitigated.** Any candidate that fails `design-evaluate` Layer-1 (contrast/token-drift/a11y) is eliminated before taste judgment. A model cannot argue past a measured contrast failure. The council judges only candidates that already cleared the floor.
