...
`design-generate` produced N directions from different taste engines. The council
picks the winner and the grafts.

0. **Read the ledger first.** `<project>/.design/council/ledger.md` + the latest
...
1. **Floor-clean is a HARD PRECONDITION for esteem scoring (R1).** For each candidate,
   read its `design-evaluate` Layer-1 artifacts. A candidate may be scored ONLY if
...
2. **Confirm renders exist** for every surviving candidate at every viewport+mode. If
   missing → route to `design-render`, then resume.
...
7. **Write the per-juror records, then the verdict record** (Phase 2.5, always).
   ...
   `design-council/verdict@1` JSON + a ledger line — shapes in the `design` skill's
...
- The R1 precondition still binds: a surface whose shipped HARD Layer-1 gates did not
  ...
  failure and route to `design-fix`; do not issue a taste critique of a broken surface.
...
- This is the Layer-2 advisory pass that `design-evaluate` defers to. It never
...
scripts:
- Render PNGs: `${CLAUDE_PLUGIN_ROOT}/skills/design-render/scripts/render.mjs`
...
- Machine floor: `${CLAUDE_PLUGIN_ROOT}/skills/design-evaluate/scripts/static-gates.mjs`
