# Running design-storm — auth, cost, corpus, re-targeting

The engine + adapter ship at `${CLAUDE_PLUGIN_ROOT}/skills/design-storm/storm/`. Everything below runs
...
`robocopy <kb>/storm <plugin>/skills/design-storm/storm /E /XD __pycache__ /XF "live-brief-*.json"`.)

## Offline first (no network, free)

```
cd "${CLAUDE_PLUGIN_ROOT}/skills/design-storm/storm"
