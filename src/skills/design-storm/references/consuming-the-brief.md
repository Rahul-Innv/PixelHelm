...
Read it before wiring any brief into `design-direction` / `design-generate` / `design-council`.
...
- **`design-ground`** folds the whole brief into the ground context as `stormBrief` (with its `abstained`
  list intact, so downstream skills see the honest gaps).
- **`design-generate`** input #1 (brief/thesis): the supported constructive recommendations + refuter risks
  ...
  constraints.
- **`design-council`**: in Phase 0 the panel reads the brief alongside the profile; the Register-fit and
