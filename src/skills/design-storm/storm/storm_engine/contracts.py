...
(no cohortwatch imports), so any project can reuse the engine by implementing the three Protocols
(FrameworkSpec, RetrievalAdapter, VerificationGate) in its own adapter.

Shapes are deliberately aligned with CohortWatch's existing research pipeline so a finished claim
...
#   framework   — derived from a FrameworkSpec dimension (CohortWatch: a research-stage lens).
...
class Angle:
    ...
    id: str
    title: str
    anchor: str                 # CohortWatch: the lens `framework_section` string
    question: str
    kind: AngleKind
