...
- `design-tokens` already owns the brand atoms (color/type/spacing/radius).
- A `frame.md` for the project is `design-ground`'s resolved contract expressed for
...
- Frames pull color/type/spacing/radius from the project token module (via
  `design-ground` / `frame.md`) — never a literal hex in the composition.
...
- The rendered output is delivered through Recipe A, so design-evaluate Layer-1
