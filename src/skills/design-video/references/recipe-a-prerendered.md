...
in JSX/CSS-in-JS they come from the project token module via `design-ground`.
...
- **Captions styling** (`::cue`) pulls from tokens — caption text/background contrast
  must clear the same WCAG floor design-evaluate enforces on body text.
...
- Poster background, the `<figure>`/player frame, focus rings, and `::cue` caption
  colors come from the project token module (via `design-ground`) — never a literal.
- Caption contrast and any on-player text are checked by `design-evaluate` Layer-1
