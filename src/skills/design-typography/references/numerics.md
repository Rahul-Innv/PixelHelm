...
`scripts/lint-typography.mjs`; `design-evaluate` Layer-1 enforces the same constants.
...
Emit findings as objects (Terrazzo style), so `design-evaluate` can fold them in:
