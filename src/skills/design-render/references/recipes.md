# design-render recipes

Concrete render flows. All commands run the pre-written
`${CLAUDE_PLUGIN_ROOT}/skills/design-render/scripts/render.mjs` — never regenerate it.
...
<!-- FULL-ONLY-START -->
- [Extreme-content re-render (the P56 machine arm)](#extreme-content-re-render-the-p56-machine-arm)
<!-- FULL-ONLY-END -->
<!-- LITE-ONLY: - [Extreme-content re-render](#extreme-content-re-render) -->
...
(The durable data dir and the full store map: `design` skill →
...
`design-generate` produces N competing directions; render ALL of them so
`design-council` and the owner pick the winner (build-spec §2: each direction is
rendered desktop+mobile, light/dark via design-render). One out-dir per direction:
...
1440/768/375 matrix for the chosen winner before design-evaluate.)

<!-- FULL-ONLY-START -->
## Extreme-content re-render (the P56 machine arm)

The council's / evaluator's **P56 Content-Robustness** composite must never judge the
flattering demo render — a load-bearing value that clips only shows under REAL-extreme
content (validated originally on a 53-char company name @ 1440 + 820px, where the
flattering render showed none of the wrap risk). Give P56 its evidence with a
<!-- FULL-ONLY-END -->
<!-- LITE-ONLY: ## Extreme-content re-render -->
<!-- LITE-ONLY: The council's / evaluator's **Content-Robustness** check must never judge the flattering demo render — a load-bearing value that clips only shows under REAL-extreme content (validated originally on a 53-char company name @ 1440 + 820px, where the flattering render showed none of the wrap risk). Give it its evidence with a -->
...
<!-- FULL-ONLY-START -->
desktop); render BOTH modes; hand P56 the extreme PNGs alongside the normal ones. If
a surface can't be re-rendered this way (a live app state you can't reproduce), P56
ABSTAINS — an abstain is "unassessed", never a PASS.
<!-- FULL-ONLY-END -->
<!-- LITE-ONLY: desktop); render BOTH modes; hand the evaluator the extreme PNGs alongside the normal ones. If a surface can't be re-rendered this way (a live app state you can't reproduce), the content-robustness check ABSTAINS — an abstain is "unassessed", never a PASS. -->

## Reduced-motion variant

When the screen animates, also capture the reduced-motion state so design-evaluate can
...
homogenize — the register differs per project; design-render captures faithfully and
...
design-render is cost-aware. After a `design-fix` that touched one viewport or one
