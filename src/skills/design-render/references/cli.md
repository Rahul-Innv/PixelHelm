...
`${CLAUDE_PLUGIN_ROOT}/skills/design-render/scripts/render.mjs`. This skill RUNS it;
...
```
node "${CLAUDE_PLUGIN_ROOT}/skills/design-render/scripts/render.mjs" <targets.json>
```
...
| `--axe` | off | Run the vendored axe-core suite on every rendered cell; per-cell `axe: {serious, critical, violations[]}` lands in `render.json` plus a run total. Report-only here — `design-evaluate` Layer-1 gates on serious/critical. Config mode: `"axe": true`. Needs `axe-core` from this skill's `npm i`. |
...
<!-- FULL-ONLY-START -->
  rendering — the P56 machine arm. See `recipes.md#extreme-content-re-render-the-p56-machine-arm`.
<!-- FULL-ONLY-END -->
<!-- LITE-ONLY: rendering — the extreme-content machine arm. See `recipes.md#extreme-content-re-render`. -->
