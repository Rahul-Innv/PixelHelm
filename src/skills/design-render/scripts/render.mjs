...
and writes serious/critical violations into render.json (design-evaluate Layer-1 treats them as
...
if (cfg.axe) {
  try { axeSource = readFileSync(resolve(dirname(fileURLToPath(import.meta.url)), "..", "node_modules", "axe-core", "axe.min.js"), "utf8"); }
    ...
    console.error("render: --axe needs axe-core for this plugin version. Fix: npm i in the design-render skill dir (axe-core is in its package.json).");
    process.exit(2);
  }
}
    ...
    for (const vp of viewports) {
      ...
      try {
        ...
        if (axeSource) {
          ...
          // design-evaluate Layer-1 owns the FAIL; serious/critical are what it gates on).
          try {
...
if (cfg.axe) console.log(!axeTotals.cellsRun
  ...
  ? `render: axe: ${axeTotals.critical} critical + ${axeTotals.serious} serious across ${axeTotals.cellsRun} cell(s) — design-evaluate Layer-1 treats these as FAIL`
