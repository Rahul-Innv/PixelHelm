...
- DOM + computed styles are captured by
  `${CLAUDE_PLUGIN_ROOT}/skills/design-render/scripts/render.mjs`, whose
  `render()` returns `{ screenshotPath, domPath, a11yTreePath, computedStyles }`.
- The deterministic Layer-1 gate that SHIPS today is
  `${CLAUDE_PLUGIN_ROOT}/skills/design-evaluate/scripts/static-gates.mjs`
...
taste.* Taste is judged by `design-council` (Layer-2), which never overrides a
