...
This is the authoritative routing reference for the `design` router. It does not
...
|---|---|---|
| `design-ground` | reference | Gate #0 on every intent except a pure LEARN — resolve contract + profile + fresh docs + lessons. |
| `design-reference` | reference | a NEW DESIGN / REDESIGN needs north-stars + a grounded brief (knowledge-as-data retrieval). |
| `design-direction` | direction | NEW DESIGN / REDESIGN and the owner has NOT mandated a look — N named directions on real data, owner picks. On the HANDS-ON branch (profile `ownerInvolvement: "hands-on"`) this is MANDATORY before any production tournament, not conditional. |
| `design-generate` | generate | a chosen direction must become production UI — runs the taste-engine tournament. |
| `design-render` | render | any candidate or existing surface must be screenshotted (both viewports, light+dark, reduced-motion). |
| `design-evaluate` | evaluate | grading is needed — Layer-1 machine gates + Layer-2 lens, with the baseline. |
| `design-council` | council | the tournament needs a winner judged, or REVIEW wants the 7-seat panel. |
| `design-fix` | fix | a Layer-1 FAIL or a [Blocker]/[High] finding must be cleared — smallest change, targeted. |
| `design-tokens` | (token source) | TOKENS intent, or any time the contract itself is created/changed. Single source of truth. |
| `design-color` | color | COLOR intent, or a contrast/palette finding. |
| `design-typography` | typography | TYPOGRAPHY intent, or a type finding. |
| `design-motion` | motion | MOTION intent, or a motion-a11y / animation finding. |
| `design-email` | email | EMAIL intent. |
| `design-dataviz` | dataviz | DATAVIZ intent, or a chart/encoding/dense-table finding. |
| `design-content` | content | CONTENT intent, or a copy/label/empty-state finding from the evaluate content phase. |
| `design-video` | video | VIDEO intent (situational; correctness floor + register trust matrix). |
| `design-storm` | reference | seated by `design-ground` on a HIGH-STAKES new design/redesign — a verified, cited brief consumed as constraints. Never every pass. |
| `design-learn` | learn | LEARN intent (standalone, user-invoked refresh), or a sibling reporting a verified correction (WRITE-BACK). |

The craft skills (`design-color`/`design-typography`/`design-motion`/`design-email`/
`design-dataviz`/`design-content`/`design-video`) also attach by concern or file path;
...
design-ground
  ...
  -> [design-reference]            (north-stars + brief, if not already briefed)
  -> [design-direction]            (if no look is mandated -> owner picks a thesis)
                                   {HANDS-ON: MANDATORY unless a look is mandated —
                                    owner picks on 2-3 throwaway mockups BEFORE any
                                    production candidate is built}
  -> design-generate               (tournament: N taste engines -> N candidates)
  -> design-render                 (each candidate: desktop+mobile, light+dark)
  -> design-evaluate               (Layer-1 eliminates failing candidates first)
  -> design-council                (judge the survivors -> winner; writes the verdict record)
  ...
  -> LOOP (iterate on the winner)  -> owner approval gate -> CLOSE THE LOOP
                                      (sign-off record; correction -> design-learn WRITE-BACK;
...
design-ground
  {HANDS-ON: ground-confirm — register + referenceSources + constraints; 1 question}
  -> design-render(current) + design-evaluate(current)   (honest baseline + diagnosis)
  ...
  -> [design-direction]            (delta thesis, if the look is open)
                                   {HANDS-ON: MANDATORY for the tournament method —
                                    owner picks the delta thesis on throwaway mockups}
  -> design-generate(delta)        (tournament on the changed surface)
  -> design-render -> design-evaluate -> design-council (verdict record written)
...
`design-render` saves to the DURABLE data dir — `<dataDir>/baselines/<project>/` (or the
...
- The same `design-render` pass that captures the baseline ALSO adds the incumbent to the
  council candidate set (same viewports + modes as the new directions).
- `design-council` scores the incumbent alongside the new directions on every lens,
...
- **Incremental polish** (good→great, already-on-register) — generate a ranked set of small,
  ...
  `${CLAUDE_PLUGIN_ROOT}/skills/design-generate/references/incremental-polish.md`. The
  incumbent guard (`references/gates-and-loop.md` Section 7) is shared by both methods.

**REVIEW / EVAL** (usable standalone, day one)
```
design-ground (light: contract + profile only)
  -> design-render
  -> design-evaluate (Layer-1 + Layer-2 + baseline classification)
  -> [design-council] (if a multi-lens panel was asked for)
...
```
design-ground
  -> design-render -> design-evaluate (confirm the finding still reproduces)
  -> design-fix    (smallest change that clears the specific finding)
  -> design-render(affected viewport) -> design-evaluate (prove it cleared, nothing regressed)
...
```
design-ground
  -> design-tokens          (the contract; single source of truth)
  -> design-color / design-typography / design-motion   (emit values INTO the contract)
  -> design-evaluate Layer-1 conformance (assert the rendered output matches the tokens)
```

**EMAIL**
```
design-ground
  -> design-email           (600px canvas, no background-dependent chips, email-safe CSS)
  -> reuse design-evaluate's contrast gate within the email-safe subset
```

**DATAVIZ / CONTENT / VIDEO** (craft intents)
```
design-ground -> design-dataviz | design-content | design-video (the craft pass)
  -> design-render -> design-evaluate     (when the pass changed a rendered surface)
```

**LEARN** (may skip Gate #0)
```
design-learn (version-stamped, expiring lessons + fingerprint-registry update; owner-reviewed)
```
