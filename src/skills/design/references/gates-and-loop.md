...
Gate #0 GROUND (`design-ground`) resolves the contract (`core <- profile <- session`),
...
`design-evaluate` runs two layers in order. The router enforces this seam when reading
...
- **Layer-1 — machine-certain gates.** No model in the loop. Honest shipping status
  ...
  - **Designed, NOT YET WIRED (no validator ships):** states-present (distinct
    ...
    token value). See `design-evaluate/references/layer-1-gates.md` for their specs.
...
`design-evaluate` prints a scope banner; the router relays it verbatim with every
evaluator result:
...
```
LOOP (after design-render + design-evaluate):
  if any Layer-1 gate FAILED, or a [Blocker] is present:
      -> design-fix (target ONLY that finding; smallest change)
      -> design-render (the affected viewport[s] only)
      -> design-evaluate (prove the finding cleared AND nothing regressed)
...
Match render cost to the change. `design-render` supports targeted re-render:
...
The router passes the change scope to `design-render` so it does not pay for a full
...
For NEW DESIGN / REDESIGN, `design-generate` produces N competing candidates. The
...
(`routing.md` REDESIGN). After `design-council` scores all candidates, the router
