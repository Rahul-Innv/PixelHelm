...
## Handoff to design-fix

`design-generate` self-corrects the WINNER against the council's verdict. If the
user then wants the broader `design-evaluate` findings turned into mutations, hand
off to `design-fix` — it owns the post-audit patch loop (rebuild-from-the-decision,
