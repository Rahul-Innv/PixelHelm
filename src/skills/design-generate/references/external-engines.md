...
2. The **token contract / design system** in whatever grounding shape the engine accepts
   (a DS asset, a design-system project, or inline constraints derived from `design-tokens`).
...
3. **Render** via `design-render` to the same matrix as every other candidate.
...
- Not a judge (the council judges), not a token source (design-tokens is), not a floor
  (design-evaluate Layer-1 is), and not required (E1–E4 run without any attachment).
