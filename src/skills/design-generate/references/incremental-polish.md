...
The incumbent guard (the `design` skill's `references/gates-and-loop.md` §7) is SHARED by both methods. Polish does not
...
3. **Render each diff** on the same matrix as the incumbent (cost-aware: a single-element
   diff re-renders only the relevant viewport/mode — the `design` skill's `references/gates-and-loop.md` §5).
...
5. **Keep a diff ONLY if it clears the shared incumbent guard** — it improves the overall
   council score AND does not lower register-fit (the `design` skill's `references/gates-and-loop.md` §7). A diff that is
