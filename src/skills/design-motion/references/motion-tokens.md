...
- [Emitting into the design-tokens module](#emit)
- [Per-register motion budget](#register)

Motion tokens are NOT a parallel config. They live in the project's ONE
`tokens.mjs` (the design-tokens seam) and emit as BOTH CSS custom properties and
...
<a id="emit"></a>
## Emitting into the design-tokens module

Motion values join the SAME `tokens.mjs` the AA-lockstep pattern governs
(design-tokens skill). Shape them as a `MOTION` block exporting durations +
easings, mirrored to a CSS `:root` block. Example (illustrative — the canonical
module is design-tokens' `tokens.template.mjs`):
