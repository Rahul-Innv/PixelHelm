# design-fix auto-rewrite catalog
...
Provenance: motion rewrites ← `design-motion` validator (build-spec §1) +
...
|---|---|---|
| Raw literal (`#fff`, `bg-white`, `border-gray-200`, `p-[16px]`) where the system has a token | Replace with the semantic token / class (`bg-card`, `border-border`, `p-4`) | Never add a new literal. If no token exists for the role, that's a design-tokens gap — escalate, don't invent. |
...
- No token exists for the needed role — that's a design-tokens gap; surface it,
