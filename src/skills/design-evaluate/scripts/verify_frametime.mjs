...
if (flags.json) {
  ...
  const L = [`design-evaluate · verify_frametime (behavior floor) · ${p.name} ${p.viewport.width}x${p.viewport.height} @ DPR ${p.deviceScaleFactor}, CPU x${p.cpuThrottleRate} · budget p95 <= ${budgetP95} ms (+${JITTER_ALLOWANCE_MS} ms fixed jitter allowance)`];
