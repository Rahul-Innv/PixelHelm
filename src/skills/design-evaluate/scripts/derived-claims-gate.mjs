...
// createRequire fallback (env override -> sibling design-render skill that vendors it
...
function requireChromium() {
  const candidates = [
    ...
    resolve(HERE, "..", "..", "design-render", "package.json"),                       // sibling skill vendors PW per plugin version
    resolve(HERE, "..", "..", "design-render", "node_modules", "playwright", "package.json"),
  ].filter(Boolean);
  ...
  die("Playwright not found. Run `npm i` in the design-render skill dir (it vendors playwright per plugin version), or set PLAYWRIGHT_PKG to a package.json that can resolve 'playwright'.");
}
