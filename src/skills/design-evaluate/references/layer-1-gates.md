...
`design-render` skill's per-version install and fail LOUDLY (exit 2) when it is missing —
...
| Anti-cliché grep | soft (reported) | No profile `bannedClusters` signature appears in source. Case-insensitive substring match per line over `sourceExts` files; includes copy tells whose needle is a character (`ai-em-dash-copy`), so comments and legitimate non-prose uses match too — the soft verdict is read by a human, never auto-actioned. |
...
| axe-core on rendered cells (opt-in) | **HARD when run** | Run `design-render`'s `--axe` arm (vendored axe-core, per rendered cell, both modes); `render.json` carries per-cell serious/critical. ANY serious/critical violation = Layer-1 FAIL. Not run ⇒ say so in the banner — never imply it passed. Covers rulebook #11 (`page-has-heading-one`/`heading-order`) and #7 (`target-size`, 24px). |
...
- **Computed-style conformance** (`getComputedStyle` === resolved token, sRGB epsilon) — designed
  (see design-tokens `references/conformance.md`), no harvester ships.
...
```
node "${CLAUDE_PLUGIN_ROOT}/skills/design-evaluate/scripts/verify_states.mjs" <file.html|url> \
...
The SHIPPED runner is `design-render`'s `--axe` arm: it injects the VENDORED axe-core (from the
...
```
node "${CLAUDE_PLUGIN_ROOT}/skills/design-evaluate/scripts/verify_focustrap.mjs" <file.html|url> \
...
```
node "${CLAUDE_PLUGIN_ROOT}/skills/design-evaluate/scripts/verify_responsive.mjs" <file.html|url> \
...
```
node "${CLAUDE_PLUGIN_ROOT}/skills/design-evaluate/scripts/verify_targetsize.mjs" <file.html|url> \
...
```
node "${CLAUDE_PLUGIN_ROOT}/skills/design-evaluate/scripts/output-floor-gate.mjs" <file.html> [--json]
```
...
```
node "${CLAUDE_PLUGIN_ROOT}/skills/design-evaluate/scripts/verify_scrollcapture.mjs" <file.html|url> \
...
```
node "${CLAUDE_PLUGIN_ROOT}/skills/design-evaluate/scripts/verify_frametime.mjs" <file.html|url> \
...
```
node "${CLAUDE_PLUGIN_ROOT}/skills/design-evaluate/scripts/verify_cwv.mjs" <file.html|url> \
...
```
node "${CLAUDE_PLUGIN_ROOT}/skills/design-evaluate/scripts/verify_keyboard.mjs" <file.html|url> \
