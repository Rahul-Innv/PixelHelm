...
DESCRIBES problems; it never prescribes CSS (that is `design-fix`). Every finding is labeled
`advisory`.
...
perfection. Use the `design-render` live backend (Playwright); never pass model-authored JS into
`browser_evaluate`.
...
1. **Interaction & user flow** — execute the primary flow; test hover/active/disabled; verify
   ...
   of the Spool lens's in-use clause, `design-council/references/lenses.md` §4.)*
...
<!-- FULL-ONLY-START -->
### Deep-pass composites (surface-triggered — the STORM-derived bench, use-case #2)
On a DEEP audit (a redesign that will ship, a high-stakes surface, or one carrying a trigger artifact),
seat the composite lenses the surface earns — a chart → **C1** Quantitative-Honesty (grounded in
`design-council/references/dataviz-canon.md`), a dense table → **C2**, a custom widget / hidden-scroll →
**C4**, an auth/consent surface → **C5**, AI-extracted or meaning-invertible copy → **C6**, a KPI
dashboard → **C7**, a first-run/empty screen → **C3**; and **always P56 / P45 / P25**. **Cap ≈ 12–13
model-judged lenses; never seat all at once.** Each is **register-subordinate** (a constraint, never the
register). Machine-certain a11y (contrast/AA/focus/target-size/reflow) stays on **Layer-1** — a composite
is only the model-judged residue and NEVER re-litigates a Layer-1 result. **P56 ABSTAINS** (not PASS)
without a real-container re-render. Full seating + severity mapping + the audit-mode aggregation analog:
**`references/layer-2-deep-pass-composites.md`** (which references the council's canonical composite
definitions — one source of truth, no drift).
<!-- FULL-ONLY-END -->
...
- Bad: "Change margin to 16px." (prescriptions belong to `design-fix`.)
