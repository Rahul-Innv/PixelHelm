...
another. Consumed by `design-evaluate` Layer-1 and the per-project profile's
...
`$CLAUDE_PLUGIN_DATA`, else `~/.claude/design/frontend-design-skill/` if it
exists, else `~/.claude/design-pixelhelm/`.
...
`design-direction` sets the surface; the matching tell-set applies. A
...
2. **The per-project profile** (`profiles/<project>.json` `bannedClusters`) pins the
   ...
   shape. design-learn proposes the diff to add/remove a cluster there.
3. **`design-evaluate` Layer-1** greps the `bannedClusters` `any[]` against source
...
diff; the owner saves it. Never mint a token here (that is `design-tokens`).
...
evidence, not on the design to defend a choice. This mirrors the design-council /
design-evaluate false-positive filter: taste ≠ defect; a bold choice ratified by
