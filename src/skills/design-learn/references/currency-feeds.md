...
3. **Verify.** Re-run the affected gates/evals (e.g. if axe-core semantics
   changed, re-run `design-evaluate` Layer-1; if a typography rule moved, re-check
...
(`<dataDir>` = `$CLAUDE_PLUGIN_DATA`, else `~/.claude/design/frontend-design-skill/`
if it exists, else `~/.claude/design-pixelhelm/`.) Update the date when REFRESH
polls a feed. Format:
