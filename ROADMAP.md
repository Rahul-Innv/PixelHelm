# PixelHelm roadmap

## Current deliberate scope

- One frontend-design capability family with atomic `pixelhelm-*` outcomes.
- Mutually exclusive Lite-default and Full-alternative editions for Claude Code.
- Thin fail-closed ChoiceGate consumption and capability inventory lifecycle authority.
- Deterministic local build, offline qualification, and compatibility metadata for the
  prior `design-*` identities.

## Before any public launch

- Complete independent criticism of the exact private-readiness candidate.
- Have the owner confirm the exact GitLab project URL and private security contact.
- Run hosted CI and remote asset checks only after separately authorized outward reads.
- Review the final public copy, license attribution, release notes, and visibility.

## Later candidates

- Separately qualify a Codex surface adapter; no safe route is claimed today.
- Add a versioned profile schema only after the profile contract is stabilized as its
  own product decision.
- Add more deterministic render fixtures and portability coverage.
- Evaluate any future marketplace or package channel separately from repository launch.

## Non-goals

- PixelHelm does not replace ChoiceGate or the external capability inventory.
- Lite and Full do not run simultaneously.
- Legacy aliases do not become natural-language owners.
- Provider calls, live installation, promotion, publication, and remote mutation are
  not implicit parts of local design work.
