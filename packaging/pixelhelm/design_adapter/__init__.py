"""pixelhelm.design_adapter — the DESIGN adapter for the portable STORM research engine.
...
into pixels as a separate step. The engine (``pixelhelm.storm_engine``) is copied verbatim and never
modified — raise engine bugs / contract changes to the upstream engine project, don't fork.

Packaging note: this directory mirrors the repository's standalone adapter at
``plugins/pixelhelm-full/skills/pixelhelm-evidence-brief/storm/design_adapter/``, with only the
engine imports rewritten to relative form so the wheel installs a single top-level package.
"""
