# Harborline repair-pass admission driver: builds the original selection request for
# the repair task, obtains the original receipt from the ACCEPTED router, builds the
# continuation, and runs the committed admit_choicegate.py verifier. Writes all
# artifacts. Adapted from the E1 admission driver
# (evals/validation/e1/run-2026-07-26/admission/driver.py); same accepted authority
# roots (see evals/validation/reacceptance-2026-07-26/RECORD.md), new request id.
import copy, hashlib, json, subprocess, sys
from pathlib import Path

REPO = Path(__file__).resolve().parents[5]
CG_ROOT = Path(sys.argv[1])  # accepted ChoiceGate worktree
INV_ROOT = Path(sys.argv[2])  # accepted Skills-OS inventory worktree
OUT = REPO / "examples/harborline/repairs/2026-07-26/admission"
...
request = {
    ...
    "request_id": "pixelhelm-harborline-repair-2026-07-26",
    "evaluation_time_utc": "2026-07-26T23:32:00Z",
    "task": {
