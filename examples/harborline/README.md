...
| `gates/` | **Committed gate-run artifacts** — every shipped floor validator executed against `status-page.html`, failures included. Three originally-committed FAILs are now cleared, two of them differently: the structural output floor and the 280/320px table overflow were cleared through the documented repair loop on 2026-07-26, and the emulated-mobile frame-time FAIL was cleared NOT by editing the page but by an owner-approved re-registration of the gate — its cause was diagnosed as a harness frame-scheduling stall, not page cost. See `gates/README.md`. |
