...
- **Sequential**: vary lightness (and a little chroma) within a single hue so "darker
  = more" reads monotonically. Generate it through pixelhelm-color's perceptual
...
- **Verify, don't eyeball.** Check the chosen data palette through pixelhelm-color's
  checks against the project token palette — CVD-safety and AA contrast are both
  pixelhelm-color's machinery; this skill consumes the verified result.

## The token roles

Add these data-channel roles to the project's pixelhelm-tokens module (not inline hex).
pixelhelm-color generates the perceptual, AA-checked values; pixelhelm-tokens stores them:
