# pixelhelm-content — per-project tone registers + honesty deference
...
> register mandate; `profiles/<project>.json` (`_register`); `pixelhelm-directions`'s
> honesty-and-registers reference. The visual register lives in pixelhelm-directions; this is its
...
`pixelhelm-directions`'s honesty bindings — these OUTRANK tone:
