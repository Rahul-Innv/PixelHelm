# pixelhelm-content — the writing process & per-surface checklists
...
4. **Build / hand off.** Write the strings into the surface using the product's real nouns,
   OR hand the rewrite to `pixelhelm-repair` to patch the JSX/DOM (this skill decides words; it does
