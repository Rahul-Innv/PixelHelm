...
(design-system priority). The token module itself is owned by `pixelhelm-tokens`;
...
- **Diagnose (pixelhelm-repair):** a "mystery offset" where `getComputedStyle` looks
...
white-label theming, and conformance all belong to `pixelhelm-tokens`. This file only
governs how generated code *consumes* that contract. If a needed color can't pass
AA, that is a `pixelhelm-color` / `pixelhelm-tokens` problem — never loosen the threshold
to ship it here.
