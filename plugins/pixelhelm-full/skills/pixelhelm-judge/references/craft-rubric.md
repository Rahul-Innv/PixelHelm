...
owner). The Craft seat grades against THIS numeric contract; `pixelhelm-repair` snaps values to it
as the minimal edit.
...
3. **Locked type scale** — hero > title > body strictly ordered; nothing functional below
   12px; no off-scale strays (15/17/19/27/33). The scale itself is `pixelhelm-typography`'s
...
- **pixelhelm-repair:** an off-ladder/off-grid finding's minimal fix is a SNAP to the nearest
