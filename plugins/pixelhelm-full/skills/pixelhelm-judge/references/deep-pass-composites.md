...
- **Machine floor is NOT a juror.** Contrast / computed-AA (both themes) / focus-trap / target-size /
  states-present / reflow are `pixelhelm-evaluate` **Layer-1** line items — measured, no model in the loop.
...
- **C8 — Persona Bench (think-aloud consumer read)** · *T2 W3 A1* · **trigger:** a consumer-facing
  ...
  `pixelhelm-directions` mockups (cheapest fix point); a mockup pass never exempts the built surface.
- **P56 — Content-Robustness under Real-Extreme Content** · *T3 W3 A3* · **trigger:** always (deep pass).
  ...
  in `pixelhelm-render` (`references/recipes.md`, the extreme-content target; validated on a 53-char company
  ...
  after-tax values, %-offs, deltas — recomputed by its author; pixelhelm-render's rule): a self-contradictory
