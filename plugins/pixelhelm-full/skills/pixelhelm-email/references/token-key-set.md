...
`pixelhelm-tokens`. This file says HOW email binds to the tokens; the contract
itself lives in `pixelhelm-tokens` — do not re-author it here.
...
values baked into the HTML. The email is registered in the `pixelhelm-tokens`
...
The `pixelhelm-tokens` lockstep test, with the email registered as `key-set`,
...
1. (lockstep, in `pixelhelm-tokens`) the email reads real, current token keys — no
...
`pixelhelm-tokens` — this skill only wires the email as a consumer of that contract.
