# pixelhelm-repair recipe — the per-finding autocorrect loop
...
Source provenance for this skill (build-spec §1 `pixelhelm-repair` ingredient table):
...
- *Problems-not-prescriptions in → patch out* — `OneRedOak design-review-agent.md`
  L68 (Layer-2 describes; pixelhelm-repair decides the CSS).
...
Each finding from pixelhelm-evaluate / pixelhelm-judge is:
...
carries the CSS value to apply — pixelhelm-repair decides it.** If a finding does carry
...
defaults** (Dammyjay93 L132). That re-derivation belongs to pixelhelm-generate or a
gated redesign. From pixelhelm-repair: report it as a clearly-named follow-up, with the
...
5. **Re-render** the touched surface via `pixelhelm-render` (`seams.md`): both
   viewports + light/dark where relevant; the reduced-motion variant if motion
   changed.
6. **Re-evaluate** via `pixelhelm-evaluate`'s gate (`seams.md`): confirm the targeted
   finding now PASSES, and diff against `.pixelhelm/baseline.json` to confirm zero
...
re-derivation of the hierarchy (which metric is the hero) → pixelhelm-generate /
...
pixelhelm-evaluate (gates prove correctness, not taste) and the count
