# pixelhelm-directions — the recipe (step-by-step + checklists + examples)
...
- **Real token names/classes** from `pixelhelm-tokens` — so porting the winner is mechanical.
...
Invoke `pixelhelm-render` (do not re-implement). It runs `render.mjs` against a real browser, shoots
...
- If the answer is **"none of these"**: run exactly **ONE** mockup-only revision round — adjust the
  ...
  "none" happens, the brief is wrong — go back to `pixelhelm-reference`, don't keep mocking.
- On a pick: hand the winning thesis + its compact token system + the mockup to `pixelhelm-generate`
