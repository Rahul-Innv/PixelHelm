# pixelhelm-directions — the taste canon (anti-slop, brief-wins, the exit tests)
...
from an adjective into a check the owner and `pixelhelm-judge` can apply to the rendered mockup:
...
Before flagging a bold choice, apply the guard (the same rule `pixelhelm-evaluate`'s
false-positive filter and `pixelhelm-judge`'s `references/false-positive-filter.md` encode).
