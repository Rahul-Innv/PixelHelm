...
`pixelhelm-color`. This file defines the ROLE STRUCTURE the token module uses; the
AA-passing hex are produced by `pixelhelm-color`, not here.
...
Do not ship frozen hand-tuned hex as the generator. Hand `pixelhelm-color` the role +
...
`node --test`. Rulings carried from `pixelhelm-color`:
...
emitted hex. The perceptual generation lives in `pixelhelm-color`; this skill is where
