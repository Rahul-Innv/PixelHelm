"""
storm/engine/gate.py — Part 2: the three-tier claim gate orchestration (PORTABLE).

Tiers (each via the injected VerificationGate; design Part II):
...
Then assemble lens sections with the honesty rules:
  ...
  - a surviving SUPPORTED disconfirming (refuter) claim -> the lens is `contested`, confidence floored
    (never averaged into a mushy middle — mirrors Concordance treating a contradiction as a cap);
...
_BASE = {"supported": 1.0, "unclear": 0.4, "unsupported": 0.0}
...
def score_claims(claims_by_run, gate: VerificationGate) -> list:
    ...
    for c in survivors:
        ...
        conf = round(_BASE.get((v or {}).get("verdict"), 0.0) * sc * cf, 2)
        scored.append(replace(c, entailment=v, corroboration=corr, confidence=conf))
    return scored


def assemble_sections(scored_claims, *, strict_drop: bool, min_support: float = 0.5) -> list[Section]:
    ...
    for lens, claims in route_claims(scored_claims).items():
        ...
        sections.append(Section(
            ...
            headline=(shipped[0].text[:90] if shipped else None),
