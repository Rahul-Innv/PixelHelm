"""
storm/engine/writer.py — Part 2: claim routing, inference-surfacing, and the anti-evasion guard.

The over-association defense (design Part II.A) lives here:
  - surface_inferences forces load-bearing "X => Y" leaps OUT of prose into their own typed `relational`
  ...
  - route_claims fans each claim to every lens in its anchor_keys (the many-to-many map; design Part II.B).
...
class InferenceSurfacer(Protocol):
    ...
    def surface(self, body: str, fact_claims: list[Claim], *, perspective_id: str) -> list[Claim]: ...
...
def assert_no_unsurfaced_inference(body: str, relational_claims: list[Claim],
                                   surfacer: InferenceSurfacer) -> list[str]:
    ...
    for detected in surfacer.detect(body):
        ...
        if not any(d in s or s in d or _overlap(d, s) >= 0.6 for s in surfaced):
