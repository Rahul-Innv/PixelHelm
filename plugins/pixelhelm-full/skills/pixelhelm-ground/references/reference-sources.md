...
pixelhelm-ground is where external design knowledge enters the loop. It enters through ONE
...
1. `<project>/.pixelhelm/references/registry.md` — a project-local registry (optional).
...
## The fetch ritual (pixelhelm-ground Procedure step 5)
...
4. **Honor staleness**: an entry past its window gets its entry-point re-verified first;
   update the entry's date and log a why-line (the pixelhelm-record-lesson currency-leg pattern — keep
...
6. **Emit `referenceSources`** into the ground context: per source consulted —
   `{ source, entryPoint, verifiedAt, tier, tookFor }`. Downstream consumers: pixelhelm-generate
   assigns arms DIFFERENT registry subsets as divergence axes (its taste-engines roster);
   pixelhelm-judge seats may CITE registry canon for grounded pattern claims.
