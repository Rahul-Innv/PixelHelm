...
every design against. No profile, no taste work: `pixelhelm-ground` refuses to
...
1. `<your-project>/.pixelhelm/profile.json` — **preferred**: commits with the
...
2. `<durable-data-dir>/profiles/<project>.json` — for projects whose repo you
   ...
   `$CLAUDE_PLUGIN_DATA` if set, else `~/.claude/pixelhelm/`.
3. Nothing found → `pixelhelm-ground` offers to CREATE `#1` from
...
- The `pixelhelm` router — reads `ownerInvolvement` at Phase 0 to pick the
  hands-on or autonomous branch (asks once when unset).
- `pixelhelm-ground` — merges the profile into every pass's ground context.
- `pixelhelm-evaluate` Layer-1 — contrast gate (tokenModule or inline pairs),
  raw-color walk (`allowRawColorIn`), cliche grep (`bannedClusters`).
- `pixelhelm-judge` — the register-fit gate binds to the literal `_register`;
  jurors may read `_taste.registerClarifications` as elaborations of it.
- `pixelhelm-record-lesson` — proposes reviewable diffs to `bannedClusters` and `_taste`;
