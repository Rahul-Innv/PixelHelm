...
mutation. Builds on the production-verified `pixelhelm/LESSONS.md` schema and
...
`~/.claude/pixelhelm/` if it exists, else `~/.claude/pixelhelm/`.
...
| a project-scoped taste/product lesson | `<project>/.pixelhelm/LESSONS.md`, or a profile `_taste` diff | travels with the project; shadows the wider layers by id |
| verified, reusable DESIGN MACHINERY (tokens/AA, email constraints, direction process) | the user's `~/.claude/pixelhelm/LESSONS.md` if they keep one (where its `pixelhelm/<topic>.md` topic docs remain a promote target), else `<dataDir>/LESSONS.md` | cross-project canon lives at the user layer |
...
| a banned/overused visual cliche | the fingerprint registry (`<dataDir>/fingerprints.md`; see `fingerprint-registry.md`) | consumed by pixelhelm-evaluate + profile bannedClusters |
...
- **skills** — routes the entry: the shared reader injects it only into the
  ...
  only into `pixelhelm` + `pixelhelm-ground`.
- **expires** — date ~90 days out; see below.
- **rule / when / source / supersedes** — as in the base `pixelhelm/LESSONS.md`.
...
but the conflict is worth a sentence (see `pixelhelm/LESSONS.md` L-009 for why).
...
- A brand-new verified rule with no SKILL.md home instead promotes into the
  relevant `pixelhelm/<topic>.md` — but only if the user keeps a global
  `~/.claude/pixelhelm/` KB; otherwise it stays a `verified` entry in
  `<dataDir>/LESSONS.md`.
...
- There are NO per-skill LESSONS files. Lessons live in the 4-layer store merged
  ...
  read-only) → (2) `~/.claude/pixelhelm/LESSONS.md` (optional user-global) →
  (3) `<dataDir>/LESSONS.md` → (4) `<project>/.pixelhelm/LESSONS.md`.
