# pixelhelm-repair seams — handoffs to render, gate, baseline, and profile

pixelhelm-repair is the mutation hinge of the loop. It consumes findings from the
...
Contents:
- [Who feeds pixelhelm-repair](#who-feeds-pixelhelm-repair)
...
## Who feeds pixelhelm-repair

- **pixelhelm-evaluate** — Layer-1 (machine gate, `static-gates.mjs`) findings name
  ...
  contract, `OneRedOak design-review-agent.md` L68). pixelhelm-repair is the skill that
  decides the value.
- **pixelhelm-judge** — consolidated multi-lens findings (deduped, severity-ranked,
  false-positive-filtered). Same shape; same contract.
- **The `pixelhelm` router** — dispatches a FIX intent here with the findings set +
  the active profile path. pixelhelm-repair does not route; it executes the loop.

Seam rule (from pixelhelm-evaluate, honored here): **Layer-2 reasoning never
...
them by cross-skill path; do NOT reimplement render or the gates inside pixelhelm-repair.

- **Re-render the touched surface** — pixelhelm-render:
  `node "${CLAUDE_PLUGIN_ROOT}/skills/pixelhelm-render/scripts/render.mjs" --target <t> --out <dir> ...`
...
- **Re-evaluate only the violation + baseline** — pixelhelm-evaluate Layer-1:
  `node "${CLAUDE_PLUGIN_ROOT}/skills/pixelhelm-evaluate/scripts/static-gates.mjs" <target>`
...
passes). Fields pixelhelm-repair relies on:
...
Nothing in this loop re-implements rendering or the gates. pixelhelm-repair owns only
