...
(The throwaway-mockup direction-pick checkpoint ships in the full edition; this edition's hands-on mode runs the ground-confirm and bar-check checkpoints.)

Whatever the mode, no work lands on real surfaces before the FINAL gate. At that
gate present:

- the rendered result (the PNG the owner can actually see — `pixelhelm-render` does the
