...
pixelhelm-render is an **adapter**: downstream skills depend on the artifact contract,
...
|---|---|---|
| screenshot | `<out>/<name>__<vp>__<mode>.png` — fullPage, deviceScaleFactor 2, fonts settled | the owner (opened via `--open`), pixelhelm-judge jurors, pixelhelm-evaluate Layer-2 |
| manifest | `<out>/render.json` — one per run: every cell + its `modeFidelity` + `extreme` flag | pixelhelm-judge (mode-fair check, H2), pixelhelm-evaluate, any orchestrator verifying the matrix is complete |
...
harvests are NOT part of the current contract — `pixelhelm-evaluate`'s machine gates
