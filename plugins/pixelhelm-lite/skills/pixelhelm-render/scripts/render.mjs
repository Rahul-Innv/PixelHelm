...
//     { "name": "screen-x-extreme", "file": "path.html",             // extreme-content re-render:
...
// the machine arm of the Content-Robustness check (see references/recipes.md).
...
for (const item of cfg.items || []) {
  ...
  if (item.url) {
    ...
    if (Array.isArray(item.substitutions) && item.substitutions.length) {
      // extreme-content arm: swap demo strings for real-extreme content, render the variant.
