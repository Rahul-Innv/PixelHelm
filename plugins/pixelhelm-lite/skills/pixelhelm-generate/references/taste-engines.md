# The taste-engine roster — pixelhelm-generate's run-time tournament
...
system-prompt persona. Each produces ONE direction; `pixelhelm-render` renders all;
`pixelhelm-judge` judges; the winner advances to the self-correct loop.
...
ground context carries `referenceSources` (pixelhelm-ground's fetch ritual), give each
...
| **E3 — North-star** | the owner's north stars (via `pixelhelm-reference`) + getdesign.md-class register exemplars — FORMAT + reasoning only, NEVER a brand payload |
...
**Source channeled:** `pixelhelm-reference` north-star corpus (the VoltAgent
...
**Distinct levers:**
- Call `pixelhelm-reference` to pull the 1-2 closest real exemplars by the surface's
  vertical + style tags.
- Imitate the exemplar's TOKEN SYSTEM — its canvas/surface lightness ladder, its
  ...
  hex.** The hex stays the project's own (from `pixelhelm-tokens`).
...
- **When the ground context carries an `extractionBrief`** (pixelhelm-ground's
  ...
  `pixelhelm-tokens`. See `pixelhelm-ground/references/design-extraction.md`.
...
1. Every direction is rendered desktop+mobile, light/dark (+ reduced-motion) via
   `pixelhelm-render`.
2. **Layer-1 machine gates run on every candidate FIRST** (`pixelhelm-evaluate`'s
...
3. Each candidate runs the **four exit tests** (Swap / Squint / Signature / Token —
   defined authoritatively in `pixelhelm-judge`'s `references/four-exit-tests.md`).
...
`pixelhelm-generate` does NOT declare the winner itself — it hands the renders to
`pixelhelm-judge`. This skill's job ends when it has produced N gate-passing,
