#!/usr/bin/env python3
"""E1 panel aggregation, exactly as pre-registered (PREREG-RUBRIC-utility.md):
...
overall median >= 8 (gate precondition recorded separately)."""
import json
from statistics import median
from pathlib import Path

base = Path(__file__).parent
m = json.load(open(base.parent / "divergence" / "blind-map.json"))
...
out = {"aggregation": "per-criterion median of 5 jurors; overall = median of the 10 criterion medians (registered)", "arms": {}}
for arm, crit in scores.items():
    ...
    out["arms"][arm] = {
        ...
        "tier3Signals": [c for c, v in medians.items() if v >= 9],
    }
...
if len(tied) == 1:
    w = out["arms"][tied[0]]
    out["passBar"] = {
        "clause1_gates": "all shipped HARD gates green + 4/4 mutant ritual recorded (see project/gates + honesty/mutant-ritual)",
