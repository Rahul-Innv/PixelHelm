...
All three arms' final candidates; PASS bar applies to the winner. Precondition:
every shipped HARD Layer-1 gate green + 4-mutant ritual recorded + the brief's
pre-registered performance budgets MEASURED and met (requires P1-5 tooling —
unmeasured budgets are a blocked precondition, never assumed).
...
limited inputs, judge-record machinery, medians) — with one addition: jurors
receive BOTH the motion renders AND the no-motion parity renders for every arm,
and criterion 1 may only be scored from the parity renders.

## PASS bar (proposed to match the owner-set bar; owner may adjust before seal)
