# E6-A admission driver: builds the original selection request for the E6-A
# motion-launch task, obtains the original receipt from the ACCEPTED router,
# builds the continuation, and runs the committed admit_choicegate.py verifier.
# Writes all artifacts. Modelled verbatim on the E1 precedent
# (evals/validation/e1/run-2026-07-26/admission/driver.py); only the request
# identity and evaluation time differ. admit_choicegate.py is NOT touched.
#
# Usage: python -B driver.py <accepted-choicegate-root> <accepted-inventory-root>
# Roots are passed as argv so no machine-absolute path is ever committed.
import copy, hashlib, json, subprocess, sys
from pathlib import Path

REPO = Path(__file__).resolve().parents[5]
CG_ROOT = Path(sys.argv[1])  # accepted ChoiceGate worktree (see reacceptance RECORD.md)
INV_ROOT = Path(sys.argv[2])  # accepted Skills-OS inventory worktree
OUT = REPO / "evals/validation/e6/run-2026-07-27/admission"
...
for p in COMPONENT_PATHS:
    content = (INV_ROOT / p).read_text(encoding="utf-8")
...
request = {
    ...
    "request_id": "pixelhelm-e6a-vireo-fold-motion-launch",
    "evaluation_time_utc": "2026-07-27T09:00:00Z",
    "task": {
    ...
    "inventory": {"schema_version": 1, "accepted_registry_commit": FAMILY["inventory"]["commit"],
