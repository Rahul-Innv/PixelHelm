# E3 admission driver: builds the original selection request for the E3 commerce
# task, obtains the original receipt from the ACCEPTED router, builds the
# continuation, and runs the committed admit_choicegate.py verifier. Writes all
# artifacts. Same protocol as E1's driver (evals/validation/e1/run-2026-07-26/
# admission/driver.py); roots via argv, path-clean.
import copy, hashlib, json, subprocess, sys
from pathlib import Path

REPO = Path(__file__).resolve().parents[5]
assert (REPO / "src/family.json").exists(), f"repo root miscomputed: {REPO}"
CG_ROOT = Path(sys.argv[1])  # accepted ChoiceGate checkout (public v0.2.1, family.json pin)
INV_ROOT = Path(sys.argv[2])  # accepted Skills-OS inventory worktree (family.json pin)
OUT = REPO / "evals/validation/e3/run-2026-07-26/admission"
...
request = {
    ...
    "request_id": "pixelhelm-e3-foxglove-fern-commerce",
    "evaluation_time_utc": "2026-07-26T23:14:00Z",
    "task": {
