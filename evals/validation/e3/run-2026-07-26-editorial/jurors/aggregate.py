#!/usr/bin/env python3
"""E3 editorial panel aggregation, exactly as pre-registered
(PREREG-RUBRIC-editorial.md, evaluator rule identical to E1's sealed rule):
...
seeded blind map. Winner = arm with the highest overall median. PASS iff every
criterion median >= 6 AND overall median >= 8 (gate precondition recorded
separately). Amendment C1 (owner-approved, pre-generation): any criterion or
overall median of 9+ is recorded as "strong, owner-verify" — never as achieved
distinctive excellence; the owner's verification pass is the binding read at
...
out = {"aggregation": "per-criterion median of 5 jurors; overall = median of the 10 criterion medians (registered)",
       "amendmentC1": "criterion/overall medians of 9+ are recorded as strong-owner-verify, never as achieved distinctive excellence",
       "arms": {}}
for arm, crit in scores.items():
    ...
    out["arms"][arm] = {
        ...
        "strongOwnerVerifySignals": [c for c, v in medians.items() if v >= 9],
    }
...
if len(tied) == 1:
    w = out["arms"][tied[0]]
    out["passBar"] = {
        "clause1_gates": "all shipped HARD gates green + 4/4 mutant ritual recorded + accessibility bar demonstrably met (see project/gates + honesty/mutant-ritual + REPORT)",
