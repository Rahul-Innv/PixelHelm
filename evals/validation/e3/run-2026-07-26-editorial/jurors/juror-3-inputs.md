...
Direction intent (written before any code existed): "Three patient broadsheet plates — when the rain comes, how much falls, how hard the worst day hits — each annotated in the reporter's own words, with the prose walking the reader from plate to plate."
...
Machine-gate outcome: All shipped machine gates GREEN for this candidate: token-contract AA recompute (text>=4.5, graphic>=3.0, light+dark); structural output floor (single main, h1, heading order, meta description); axe-core 0 serious / 0 critical on all 4 rendered cells; no horizontal overflow at 280/320/414px; state-aware contrast (default/hover/focus, both modes) pass; target-size (WCAG 2.5.8) pass; focus-trap not-applicable (no dialog); derived-claims honesty gate pass (34 year blocks scoped, both modes); required-content manifest pass (17 items, both modes). Mode fidelity verified ok for light and dark renders.

## candidate-2

Direction intent (written before any code existed): "A parish almanac read top to bottom: every year since 1990 is one ruled line whose rain marks sit on a fixed September-to-October shelf, so the eye watches the wet season walk later down the page while the entries grow heavier."
...
Direction intent (written before any code existed): "Two inks for two instruments: the manual-gauge years print in umber, the weir years in river blue, and the whole story — later, harder — is read by watching where the blue marks drift away from the umber ones."
