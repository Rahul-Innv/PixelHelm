#!/usr/bin/env python3
"""E3(saas) panel aggregation, exactly as pre-registered
(PREREG-RUBRIC-saas-marketing.md): per-criterion score = median of the 5
jurors; overall = median of the 10 criterion medians; no means, no weights.
De-blinds candidate labels via the seeded blind map (this directory's
blind-map.json). Winner = arm with the highest overall median (the tournament
...
applies at the READING layer: any 9+ median is reported as
"strong, owner-verify", never as achieved distinctive excellence."""
import json
...
out = {"aggregation": "per-criterion median of 5 jurors; overall = median of the 10 criterion medians (registered); Amendment C1: 9+ medians read as strong-owner-verify", "arms": {}}
...
if len(tied) == 1:
    w = out["arms"][tied[0]]
    out["passBar"] = {
        "clause1_gates": "all shipped HARD gates green + 4/4 mutant ritual recorded (see project/gates + project/honesty/mutant-ritual)",
        ...
        "readingRule_C1": "9+ medians are 'strong, owner-verify' — the owner's verification pass is the binding read at the top of the scale",
    }
