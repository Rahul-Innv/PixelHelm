# E3(saas) admission driver: builds the original selection request for the E3
# Ledgerline task, obtains the original receipt from the ACCEPTED router, builds
# the continuation, and runs the committed admit_choicegate.py verifier. Writes
# all artifacts. Same pattern as the E1 driver (e1/run-2026-07-26/admission/);
# committed at run-dir depth, hence parents[5] for the repo root.
import copy, hashlib, json, subprocess, sys
from pathlib import Path

REPO = Path(__file__).resolve().parents[5]
CG_ROOT = Path(sys.argv[1])  # accepted ChoiceGate v0.2.1 worktree (see e1 reacceptance RECORD.md)
INV_ROOT = Path(sys.argv[2])  # accepted Skills-OS inventory worktree
OUT = REPO / "evals/validation/e3/run-2026-07-26-saas/admission"
...
request = {
    ...
    "request_id": "pixelhelm-e3-ledgerline-saas-marketing",
    "evaluation_time_utc": "2026-07-26T23:09:00Z",
    "task": {
